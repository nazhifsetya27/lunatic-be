...
exports.detailData = async (req) => {
  ...
  console.log(detailData?.price, '<<<ass')
...
exports.exampleData = async (req, res) => {
  ...
  const WsUmum = wb.addWorksheet('Umum')
  ...
  const validations = {
    ...
    Unit: () => {
      ...
      if (unitUser?.name) {
        WsUmum.cell(2, 4).string(unitUser.name) // Place default value in row 2, column 4 (D2)
      }
    },
    Building: () => {
      const column = 'E'
  ...
  Object.entries(validations).forEach(([key, validation], index) => {
    WsUmum.cell(1, index + 1).string(key)
    validation()
  })
  WsUmum.row(1).freeze()
  ...
  wb.write('example-import-umum.xlsx', res)
}

exports.importData = async (req) => {
  const { umums } = req.body
  if (!umums?.length) throw 'Data is empty'
  ...
  async function startImport() {
    ...
    let index = 0
    await async.eachSeries(umums, async (value) => {
      ...
      try {
        await sequelize.transaction(async (transaction) => {
          ...
          if (!element.Conditions) element.errors.push('Condition is required')
            ...
            umum.condition_id = condition.id
          }
          ...
          if (
            ...
            if (!storagemanagement) {
              ...
              umum.storage_management_id = storagemanagement.id
            }
          }

          // Create or update Furniture
          if (!element.errors.length) {
            const [newUmum, created] = await Models.Asset.findOrCreate({
              req,
              transaction,
              paranoid: false,
              defaults: umum,
              where: { name: umum.name, category: 'Umum' },
            })

            if (!created) {
              await newUmum.update(umum, { req, transaction })
            }
          }
        })
      } catch (error) {
      ...
      const progress = Math.ceil(((index + 1) / umum.length) * 100)
